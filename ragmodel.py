...
genai.configure(api_key="AIzaSyBTwmwLd3S8E5X0yuzvGFbT4sUKsZH07-I")
...
st.subheader(':green[Your Intelligent Documnet Assistement 📌👤]')
...
if uploaded_file:
    ...
    if raw_text.strip(): # ensures the pdf is not empty, removes spaces and check any content exist
        ...
        chunk_text = splitter.split_documents([doc]) # splits the documnet into multiple smaller chunks
        ...
        if query:
            with st.chat_message('human'): # display msg n chat format
                with st.spinner('Analyzing the documents.....'):
                    relevent_data = retrive.invoke(query) # find the most similar cext chunks using FAISS
